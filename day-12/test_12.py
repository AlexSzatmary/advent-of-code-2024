...
import os
import pytest
from run_12 import parse, garden_fill, tally_costs, solve_1, array_to_str
...
def test_garden_fill(
    ...
    garden = garden_1
    # A
    garden, area, perimeter = garden_fill(garden, 1, 2)
    ...
    assert perimeter == 10
    # B
    garden, area, perimeter = garden_fill(garden, 2, 2)
    assert area == 4
    assert perimeter == 8
    # C
    garden, area, perimeter = garden_fill(garden, 3, 4)
    assert area == 4
    assert perimeter == 10
    # D
    garden, area, perimeter = garden_fill(garden, 2, 4)
    assert area == 1
    assert perimeter == 4
    # E
    garden, area, perimeter = garden_fill(garden, 4, 3)
    assert area == 3
    assert perimeter == 8

    _, area, perimeter = garden_fill(garden_2, 1, 2)
    assert area == 21
    assert perimeter == 36


def test_tally_costs(
    garden_1: np.ndarray, garden_2: np.ndarray, garden_3: np.ndarray
) -> None:
    costs_1 = tally_costs(garden_1)
    assert [costs_1[c] for c in "ABCDE"] == [40, 32, 40, 4, 24]
    costs_3 = tally_costs(garden_3)
    ...
    # for a, b in zip([costs_3[c] for c in ], reference):
    #     print(a, b)
    # assert [costs_3[c] for c in "RICFVJCEIMS"] == reference
